...
exports.createTransaction = [
  auth,
  body("transactionType", "Transaction Type must not be empty.")
    .isLength({ min: 1 })
    .trim(),
  ...
  (req, res) => {
    try {
      ...
      Transaction.findById(req.params.id, (err, foundTransaction) => {
        ...
        } else if (foundTransaction.status === "verified") {
          return apiResponse.ErrorResponse(res, "Transaction Already Verified");
        } else {
